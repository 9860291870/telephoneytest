...
class MainActivity : AppCompatActivity() {
    ...
    fun Send(v: View) {
        ...
        for (num in list) {
            ...
            var deliverIntent = Intent(this, DeliverActivity::class.java)
            var psIntent = PendingIntent.getActivity(this, 0, sendIntent, 0)
            var pdIntent = PendingIntent.getActivity(this, 0, deliverIntent, 0)
            var sManager = SmsManager.getDefault()
            sManager.sendTextMessage(num, null, et2?.text.toString(), psIntent, pdIntent)
    ...
    fun sendMail(v:View){
        ...
        i.putExtra(Intent.EXTRA_SUBJECT, arrayOf(et4?.text.toString()))
        i.putExtra(Intent.EXTRA_TEXT, arrayOf(et5?.text.toString()))
